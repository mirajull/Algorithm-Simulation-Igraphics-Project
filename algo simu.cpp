...
void allClear()
{
	mode=0  ; toss=0;
				...
				permitFlagN=0; pause = 0 ; flagN=0 ; doneBFS[11]; 
...
void iDrawNodesBFS(int E , int N)
{
	for(i=1 ; i<= N ;i ++)
	{
		...
		if(bfsMode==4 && permitFlagN==0 ){ iSetTimer(2000,permitsetterBFSN) ; permitFlagN=1 ;}
...
//			if(permit[i]==1 && permit[x1]==1 && level[i]!=level[x1]  ) // && ( doneBFS[i]==0 || doneBFS[x1]==0) ) 
//			{
//				
//				iSetColor(255,0,0) ;
//			}
...
//				for(k=-5;k<=5 ; k++)
//				{
//					iLine(a[i][0]+k,a[i][1]+k,a[x1][0]+k,a[x1][1]+k );
//			
//				}
//		}
//	}
...
void dfsThirdPage( void )
{
	...
	iFilledRectangle(0,0,screen_x,screen_y);//full screen white
	iDrawGraphDFS(E,N) ;
	if( bfsMode==4 ) { if(calldfs==0){ dfs(source ) ; calldfs=1 ;  for(i=0 ; permitLine[i] ;i++) printf("\nDFS NODE %i %d",i,permitLine[i]) ; }  }
	//iSetTimer(3,iDrawGraph);
}
...
int main()
{
	...
	iInitialize(1000, 600, "algorithm simulation");
	return 0;
}