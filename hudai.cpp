...
void iDrawNodesDFS(int E , int N)
{
	...
	for(i=1 ; i<= N ;i ++)
	{
		...
		if(bfsMode==4 && permitFlagN==0 ){ iSetTimer(2000,permitsetterDFSN) ; permitFlagN=1 ;}
...
void iDrawEdgeBFS(int E,int N) // for C++ not C
{
	for(i=1 ; i<= N ; i++)
	{
		...
		for(j=1 ; j<=N ; j++)
		{
			...
			if(permit[i]==1 && permit[j]==1 && level[i]!=level[j] && x1 ) // && ( doneBFS[i]==0 || doneBFS[x1]==0) ) 
			{
...
int main()
{
	...
	iInitialize(1000, 700, "algorithm simulation");
	return 0;
}